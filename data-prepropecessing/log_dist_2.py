...
file_path = '/Users/jaspuurrrr/DATASCI/binned_rank_streams.csv'
data = pd.read_csv(file_path, low_memory=False)

# Step 2: Extract the "streams" column
days = data['days_on_chart'].dropna()  # Drop any missing values

# Convert the "streams" column to a numpy array
days = days.to_numpy()

# Step 3: Apply log transformation
days_log = np.log1p(days)  # Use np.log1p to handle zeros in the data
...
plt.hist(days_log, bins=100, color='blue', edgecolor='black', alpha=0.7)
...
categories = pd.cut(days_log, bins=bins, labels=labels, include_lowest=True)

# Add the categories to the original DataFrame
data['days_on_chart_category'] = categories

# Print a sample of the categorized data
print(data[['days_on_chart', 'days_on_chart_category']].head())
...
plt.figure(figsize=(10, 6))
plt.hist(days_log, bins=bins, color='blue', edgecolor='black', alpha=0.7)
